...
function getImagesInDir($dir, $relativePath = '', $allowedExts, $rootDir, $thumbDir, $thumbWidth, $thumbHeight) {
    ...
    foreach ($items as $item) {
        ...
        if (is_file($fullPath) && in_array($ext, $allowedExts)) {
            ...
            if($GLOBALS['loginUser'] === 'user')
            {
                session_start();
                $userifadmin = $_SESSION['user_info']['username'] . '/' . $relativeFullPath;
            }else{
                $userifadmin = $relativeFullPath;
            }
            // 2. 构建图片信息数组 （单页数据）
            $images[] = [
                'name' => $item,
...
function getImagesByPage($allImages, $page, $pageSize, $groupBy) {
    ...
    return [
        ...
        'hasMore' => $page < $totalPages,// 是否有更多页
...
if ($isAjax) {
        ...
        foreach ($images as $image) {
            ...
            $html .= '<div class="image-item" 
                ...
                <div class="image-actions">
                    <button class="action-btn delete-btn" onclick="event.stopPropagation(); deleteImage(\'' . htmlspecialchars($image['file_url']) . '\', \'' . htmlspecialchars($image['thumb_url']) . '\')">删除</button>
                    <button class="action-btn move-btn" onclick="event.stopPropagation(); openMoveModal(\'' . htmlspecialchars($image['file_url']) . '\')">移动</button>
                    <button class="action-btn rename-btn" onclick="event.stopPropagation(); openRenameModal(\'' . htmlspecialchars($image['file_url']) . '\', \'' . htmlspecialchars($image['name']) . '\')">修改</button>
                </div>
            </div>';
        }
    ...
    echo json_encode([
        'html' => $html,
        'hasMore' => $pageResult['hasMore'],// 是否有更多页
